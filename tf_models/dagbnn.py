...
class DAGBNN:
    ...
    def __init__(self, params):
        ...
        print('[ DAGBNN ] Initializing model: {} | {} networks | {} elites'.format(params['name'], params['num_networks'], params['num_elites']))
    ...
    def _save_best(self, epoch, holdout_losses):
        ...
        if self._epochs_since_update > self._max_epochs_since_update:
            # print('[ DAGBNN ] Breaking at epoch {}: {} epochs since update ({} max)'.format(epoch, self._epochs_since_update, self._max_epochs_since_update))
            return True
        else:
            return False
    ...
    def train(self, inputs, targets,
        ...
        print('[ DAGBNN ] Training {} | Holdout: {}'.format(inputs.shape, holdout_inputs.shape))
        with self.sess.as_default():
        ...
        progress.stamp()
        if timer: timer.stamp('dagbnn_train')

        self._set_state()
        if timer: timer.stamp('dagbnn_set_state')
        ...
        if timer: timer.stamp('dagbnn_holdout')

        self._end_train(holdout_losses)
        if timer: timer.stamp('dagbnn_end')

        val_loss = (np.sort(holdout_losses)[:self.num_elites]).mean()
        model_metrics = {'val_loss': val_loss}
        print('[ DAGBNN ] Holdout', np.sort(holdout_losses), model_metrics)
    ...
    def _compile_outputs(self, inputs, ret_log_var=False):
        ...
        for layer in self.layers:
            if not layer.is_last_layer:
                ...
                cur_out = layer.compute_output_tensor(cur_out, inputs)
