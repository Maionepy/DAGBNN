...
from predict_env_dagbnn import PredictEnv
from sample_env import EnvSampler
from tf_models.constructor_dagbnn import construct_model
...
def readParser():
    ...
    parser.add_argument('--rollout_fixed', default=False, action="store_true",
                        help='rollout length fixed during training (default: False)')
...
def train(args, env_sampler, predict_env, agent, env_pool, model_pool):
    ...
    for epoch_step in range(args.num_epoch):
        print(f"{epoch_step}/{args.num_epoch}")
        ...
        for i in count():
            ...
            cur_state, action, action_1, next_state, reward, done, info = env_sampler.sample(agent)

            env_pool.push(cur_state, action, action_1, reward, next_state, done)
            ...
            if total_step % args.epoch_length == 0:
                ...
                while (not done) and (test_step != args.max_path_length):
                    cur_state, action, action_1, next_state, reward, done, info = env_sampler.sample(agent, eval_t=True)
                    sum_reward += reward
                    test_step += 1

                logging.info("Step Reward: " + str(total_step) + " " + str(sum_reward))
                print(f"Sum reward: {sum_reward}")
                avg_reward['x'].append(epoch_step+1)
                avg_reward['y'].append(sum_reward)
    filehandler = open(f"elements/{args.env_name}_reward_DAGBNN_v1.pkl","wb")
    pickle.dump(avg_reward,filehandler)
    filehandler.close()
...
def train_predict_model(args, env_pool, predict_env):
    ...
    state, action, action_1, reward, next_state, done = env_pool.sample(len(env_pool))
    delta_state = next_state - state
    inputs = np.concatenate((state, action, action_1), axis=-1)
...
def rollout_model(args, predict_env, agent, model_pool, env_pool, rollout_length):
    state, action, action_1, reward, next_state, done = env_pool.sample_all_batch(args.rollout_batch_size)
    print(f"Rollout length:{rollout_length}")
    for i in range(rollout_length):
        print(f"{i+1}/{rollout_length}")
        # TODO: Get a batch of actions
        action = agent.select_action(state)
        next_states, rewards, terminals, info = predict_env.step(state, action, action_1)
        # TODO: Push a batch of samples

        model_pool.push_batch([(state[j], action[j], action_1[j], rewards[j], next_states[j], terminals[j]) for j in range(state.shape[0])])
        nonterm_mask = ~terminals.squeeze(-1)
        if nonterm_mask.sum() == 0:
            break
        state = next_states[nonterm_mask]
        action_1 = action[nonterm_mask]
...
def train_policy_repeats(args, total_step, train_step, cur_step, env_pool, model_pool, agent):
    ...
    for i in range(args.num_train_repeat):
        ...
        env_state, env_action, env_action_1, env_reward, env_next_state, env_done = env_pool.sample(int(env_batch_size))

        if model_batch_size > 0 and len(model_pool) > 0:
            model_state, model_action, model_action_1, model_reward, model_next_state, model_done = model_pool.sample_all_batch(int(model_batch_size))
...
def main(args=None):
    ...
    g_bar_size = int(state_size*action_size)
    ...
    # Save agent!
    agent.save_model(args.env_name + "_v1_epochs_" + str(args.num_epoch))
